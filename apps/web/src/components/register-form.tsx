...
export default function RegisterForm({ onSubmit, isPending }: Props) {
  const form = useForm<RegisterFormData>({ resolver: validate(registerSchema) });
    ...
    <form className="space-y-4" onSubmit={form.handleSubmit(onSubmit)}>
      <div className="text-dim flex flex-col gap-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Start your interview prep journey</p>
        </div>
        {INPUTS.map((input) => (
      ...
      <p className="mt-5 text-center text-sm text-dim">
        ...
        <Link href="/login" className="font-semibold text-lime-cs hover:opacity-80 transition-opacity">
          Sign in
        </Link>
      </p>
    </form>
